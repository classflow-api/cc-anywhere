...
完整的三端部署与首次使用流程。如果只想快速体验，主 [README](/#-快速开始) 的简版即可；这里覆盖：
...
> 由于 Server 是 [dumb proxy 设计](/guide/architecture#dumb-proxy-server)，Mac / Android 升级不要求 Server 同步升级。Server 只在以下情况才需要重部：鉴权、设备管理、image 上传、TLS、依赖升级。
