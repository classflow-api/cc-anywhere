...
主要协作模式：项目大部分代码由 [Claude Code](https://claude.com/claude-code) 辅助完成（[完整开发流程文档](./AskUserQuestion远程交互/) 可供参考），由维护者审查 + 拍板。
...
加新业务 type **不需要重新部署 Server**。详见 [ARCHITECTURE.md §3.3](./ARCHITECTURE.md#33-dumb-proxy-server)。
...
更多见 [INSTALL.md - 故障排查](./INSTALL.md#故障排查) 章节。
...
证明 hook 链路出问题或主动降级。按 [INSTALL.md 故障排查](./INSTALL.md#claude-tui-仍然弹原-askuserquestion-弹窗)。
